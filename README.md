...
- **클라우드 기능(webspeech·qwen3)은 `gateOptional`** — 초과해도 WARN이고 전체 판정과 기준선
  승격에서 제외된다. 실제로 대량 호출 후 partial만 오고 final이 끊겨 CER이 100%로 튀었는데
  코드와 무관했다(수정 전 코드로도 재현). 외부 서비스 상태를 커밋 통과 조건으로 삼으면 게이트가
  신뢰를 잃는다. 대신 결과지에 WARN으로 눈에 띄게 남는다.
